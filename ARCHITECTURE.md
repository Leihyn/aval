...
└── screenshots/
    ...
    └── tests.png     # 24 passing                                           (demo phase)
```
...
3. `test/inflight.test.ts` → gates everything downstream; a red suite means stop. **DONE, 24/24.**
...
| 2 | Test suite | `cd contract && npm install && npm test` | exit 0, "24 passed" | 1 | none |
...
|---|---|---|---|---|---|
| Test suite | compiled contract | in-process import | none | `npm test` → 24 passed | P0 |
