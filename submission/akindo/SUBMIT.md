...
8 disclose() call sites bridge private to public, each annotated in source with what it reveals. The public ledger records a Merkle root, a nullifier and a fill count. It never records an amount.

Three mechanisms make it an instrument rather than a statement: single use (the nullifier), counterparty bound (the payee is in the leaf), expiring (ledger time). Settlement needs all three; a balance claim needs none.
